...
class MySQLDatabase{
	...
	query(query_string){
		...
		return new Promise((resolve, reject) => {
			...
			if(db_params.debug) console.info("Running Query",query_string);
	...
	find(db_params,query){
		...
		if(query){
			...
			where = keys.map(x => `${x}='${query[x]}'`).join(' and ');
		}
		let query_string = `SELECT ${projection} FROM ${db_params.table} WHERE ${pool.escape(where)} LIMIT ${limit}`
		if(db_params.debug) console.info("Prepared Query",query_string);
		return new Promise((resolve, reject) => {
			if(query_string === undefined) reject("No Query Provided!");
			pool.query(query_string,(err, rows, fields) => {
				if(err) reject(err);
	...
	async updateMany(db_params, query={}, doc){
		...
		let query_string = `UPDATE test SET ${assignments} WHERE ${where}`;
